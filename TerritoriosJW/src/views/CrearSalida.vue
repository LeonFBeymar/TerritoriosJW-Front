...
const { puntos: puntosDelTerritorio, cargandoPuntos } = usePuntosDeTerritorio(territorioSeleccionado);
const { progresoDias, cargandoProgreso } = useProgresoSistemaDias(territorioSeleccionado);
const { conflicto, verificando, verificar: verificarDisponibilidad } = useDisponibilidadSalida(form);
...
// Al cambiar de territorio cambian sus puntos de encuentro, así que se limpia la selección previa.
...
const crearSalida = async () => {
  ...
  // Fecha/hora en formato ISO local sin conversión de zona
  ...
  if (salidaStore.error) {
    notificarError(salidaStore.error);
    return;
  }
    ...
    <form @submit.prevent="crearSalida" class="row g-3">
      ...
      <div class="col-12">
        ...
        <div v-else-if="puntosDelTerritorio.length > 0" class="form-check mt-2">
          <input class="form-check-input" type="checkbox" id="puntoManualCrear" v-model="puntoManual">
          <label class="form-check-label small" for="puntoManualCrear">Escribir otro punto</label>
        </div>
      </div>
